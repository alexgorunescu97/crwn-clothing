...
export type SignUpSucess = ActionWithPayload<USER_ACTION_TYPES.SIGN_UP_SUCCESS, { user: User; additionalInformation: AdditionalInformation; }>
...
export const signUpStart = withMatcher((displayName: string, email: string, password: string): SignUpStart => createAction(USER_ACTION_TYPES.SIGN_UP_START, { email, password, displayName }));

export const signUpSuccess = withMatcher((user: User, additionalInformation: AdditionalInformation): SignUpSucess => createAction(USER_ACTION_TYPES.SIGN_UP_SUCCESS, { user, additionalInformation }));
